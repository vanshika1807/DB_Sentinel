  ...
  return (
    <div className="p-6 font-sans min-h-screen bg-gradient-to-b from-gray-50 to-gray-100">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px,1fr))', gap: 12, marginTop: 16 }}>
        ...
        <StatCard label="Slow Queries (total)" value={latest?.slowQueriesTotal} icon="/assets/slow.png" />
        <StatCard label="Avg Query Time (ms)" value={fmt(latest?.avgQueryTimeMs, 0)} icon="/assets/average.png"/>
        <StatCard label="CPU (%)" value={fmt(latest?.systemCpuLoadPct, 1)} icon="/assets/cpu.png"/>
        <StatCard label="Free / Total" value={bytes(latest?.freeMemoryBytes) + ' / ' + bytes(latest?.totalMemoryBytes)} icon="/assets/total.png" />
      </div>
